...
int main(int argc, char **argv)
{
  ...
  // Provide the correct syntax of this analysis code
    printf("Correct syntax: md.analysis.cxx inputfile outputfile inputparameter(atom id that you want to print its xyz) cellsize speciesA[center-atom-type] speciesB[angle-atom-type] bond-cutoff\n");
    exit(0);
  }
  else {
...
//printf("comparei:%d  comparej:%d\n",comparei,comparej);
					...
					zi=myatom[comparei]->x[2];zj=myatom[comparej]->x[2];
                                        ...
                                        while (dz<-lz/2.0){dz=dz+lz;}
                                        dist=sqrt(dx*dx+dy*dy+dz*dz);// printf("%d-%d:type%d-%d:dist %f ",myatom[comparei]->id,myatom[comparej]->id,itype,jtype,dist);
					...
					if(dist<=bondlength) neighbours.push_back(comparej); //printf("atomID %d\n",comparej);
					}
	              			}	
					} 
					...
					angle=bondangle(myatom[neighbours[il]],myatom[comparei],myatom[neighbours[jl]],lx,ly,lz);
                                        index=int(angle/dr); //printf("angle=%f,index=%d ",angle,index);
                                        if(index<=nbins){anglenumber++;bin[index]++;//printf("bin%d=%d\n",index,bin[index]);
					}
